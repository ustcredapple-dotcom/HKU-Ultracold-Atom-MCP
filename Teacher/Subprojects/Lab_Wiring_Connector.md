...
- 添加工程内自定义端口种类。
- 端口会根据端口种类自动显示颜色；未连接端口为空心圆，已被连接占用的端口为实心圆。
- 连线端点锚定到真实可见的端口圆点中心，而不是只锚定到器件方块边缘。长线主路径在器件卡片外侧走线，端口圆点到卡片外侧之间由同色短引线在器件上方显示，避免密集端口看起来连错接口或被方块压住。
- 连接在物理意义上按无方向理解。工程 JSON 中仍保留 `from` 和 `to` 字段用于兼容 schema，但网页和 AI 摘要显示为 `接口 A`、`接口 B`，连接符使用 `--` 而不是箭头。
- 点击两个端口后选择线型，创建连接。
- 自动整理器件布局。
...
网页顶部 `整理 / Arrange` 会根据连接关系自动布置器件:

- 有连接关系时，将连接网络的两端按层排开，减少线缆交叉；这只是布局策略，不代表物理信号方向。
- 无连接关系时，按网格排列。
- 大量端口的器件会保留更高的纵向空间。
